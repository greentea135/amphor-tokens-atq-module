...
const SUBGRAPH_URLS: Record<string, { decentralized: string }> = {
  ...
  "1": {
    decentralized:
      "https://gateway.thegraph.com/api/[api-key]/subgraphs/id/RkDsVce2tRzGWbNAeZJrRy7nLAmMWtxQ6ixerhhw581",
  },
};
...
function transformPoolsToTags(chainId: string, pools: Pool[]): ContractTag[] {
  ...
  return validPools.map((pool) => {
    ...
    return {
      ...
      "Project Name": "Amphor",
      "UI/Website Link": "https://amphor.io/",
      "Public Note": `Amphor's ${pool.outputToken.symbol} (${pool.outputToken.name}) token contract.`,
    };
  });
}
